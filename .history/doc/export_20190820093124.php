...
$begin = $_GET['begin'];
$end = $_GET['end'];
$startDate=$begin." 00:00:00";
$endDate=$begin." 23:59:59";
$sql = "select name,atime,status,ip from login_history where atime >='$startDate' and  atime <='$endDate' order by atime desc ";
$history = get_fetchAll_assoc($project, $sql);
$title = array("用户", "时间", "状态", "ip");
$fileName = $begin . '到' . $end . '登录记录';
$res = toExcel($history, $title, $fileName, 'php://output');
