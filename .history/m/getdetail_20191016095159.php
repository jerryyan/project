...
if ($_POST['type'] == 'zjdd') {
    ...
    $nnwc_jz_sql = "select a.ts,convert(varchar(10),a.ts,108) as tstime  from info_registered as a left join info_appointment b on a.AppointmentId=b.AppointmentId where rdepartment not in ('未预约','耳鼻喉科') and a.dr=0 and isappointment=1 and rsourceType !='自然门诊' and rsymptom !='体检' and gn like '%胃肠%' and a.ts >'{$startDate}' and a.ts <='{$endDate}'";
    ...
    $nnwc_yy_sql = "select department,ts,awr,convert(varchar(10),ts,108) as tstime  from info_appointment where department not like '%未预约%' and dr=0  and sourceType !='自然门诊' and symptom !='体检'  and gn like '%胃肠%' and ts >='{$startDate}' and ts<='{$endDate}'";
    $nnwc_yy_row = get_fetchAll_assoc($nndd, $nnwc_yy_sql);
    $nnwc_yyNum = getAllNum($nnwc_yy_row, $date);
    $nnwc_jz_sql = "select a.ts,convert(varchar(10),a.ts,108) as tstime  from info_registered as a left join info_appointment b on a.AppointmentId=b.AppointmentId where rdepartment not in ('未预约','耳鼻喉科') and a.dr=0 and isappointment=1 and rsourceType !='自然门诊' and rsymptom !='体检' and gn like '%胃肠%' and a.ts >'{$startDate}' and a.ts <='{$endDate}'";
    $nnwc_jz_row = get_fetchAll_assoc($nndd, $nnwc_jz_sql);
