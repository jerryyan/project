...
if ($_POST['type'] == 'zjdd') {
    ...
    $szty_yy_sql = "select department,ts,awr,convert(varchar(10),ts,108) as tstime  from info_appointment where department not like '%未预约%' and dr=0  and ts >='{$startDate}' and ts<='{$endDate}'";
    $szty_yy_row = get_fetchAll_assoc($szty, $szty_yy_sql);
    $szty_yyNum = getAllNum($szty_yy_row, $date);
    $szty_jz_sql = "select a.ts,convert(varchar(10),a.ts,108) as tstime  from info_registered as a left join info_appointment b on a.AppointmentId=b.AppointmentId where rdepartment not like '%未预约%' and a.dr=0 and isappointment=1 and awr is not null and a.ts >'{$startDate}' and a.ts <='{$endDate}'";
    $szty_szty_jz_row_row = get_fetchAll_assoc($szty, $szty_yy_sql);
    $szty_jzNum = getAllNum($szty_jz_row, $date);
    $data['yyNum'] = $szty_yyNum;
    $data['jzNum'] = $szty_jzNum;
}
