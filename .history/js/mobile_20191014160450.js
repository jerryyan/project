...
function getyySource(datetime) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var type = getType();
            //设置天元的预约数据
            $(".ty_1 span").text("全天 ：" + data.tydd_yyNum.totalNum);
            $(".ty_2 span").text(type + data.tydd_yyNum.workNum);
            $(".ty_3 span").text("目标 ：" + tydd_target);
            var tydd_total_height = GetPercent(data.tydd_yyNum.totalNum, long);
            var tydd_work_height = GetPercent(data.tydd_yyNum.workNum, long);
