...
function getjzSource(datetime) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var type = getType();
            //设置天元的就诊数据
            $(".ty_1 span").text("全天 ：" + data.tydd_jzNum.totalNum);
            $(".ty_2 span").text(type + data.tydd_jzNum.workNum);
            $(".ty_3 span").text("目标 ：" + tydd_target);
            var tydd_total_height = GetPercent(data.tydd_jzNum.totalNum, long);
            var tydd_work_height = GetPercent(data.tydd_jzNum.workNum, long);
            var tydd_target_height = GetPercent(zjdd_target, long);
            $(".ty_3").css("height", tydd_target_height);
            $(".ty_1").css("height", tydd_total_height);
            $(".ty_2").css("height", tydd_work_height);
            //设置湛江的就诊数据
            $(".ty_1 span").text("全天 ：" + data.zjdd_jzNum.totalNum);
            $(".ty_2 span").text(type + data.zjdd_jzNum.workNum);
            $(".ty_3 span").text("目标 ：" + zjdd_target);
            var zjdd_total_height = GetPercent(data.zjdd_jzNum.totalNum, long);
            var zjdd_work_height = GetPercent(data.zjdd_jzNum.workNum, long);
            var zjdd_target_height = GetPercent(zjdd_target, long);
            $(".ty_3").css("height", zjdd_target_height);
            $(".ty_1").css("height", zjdd_total_height);
            $(".ty_2").css("height", zjdd_work_height);
